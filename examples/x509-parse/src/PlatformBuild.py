...
class X509ParseSettingsManager(
    UpdateSettingsManager, SetupSettingsManager, BuildSettingsManager
):
    """
    Settings manager for X509Parse EFI application build
    """
...
class PlatformBuilder(UefiBuilder):
    """
    Platform build for X509Parse module
    """
    ...
    def SetPlatformEnv(self) -> int:
        ...
        """
        self.env.SetValue(
            "ACTIVE_PLATFORM", "X509Parse/X509Parse.dsc", "Platform hardcoded"
        )
        self.env.SetValue("PRODUCT_NAME", "X509Parse", "Platform hardcoded")
