...
#define __cpuid(level, sublevel, a, b, c, d)                \
...
#define __arch_harness_start(addr_ptr, size_ptr)                          \
  do {                                                                    \
    uint32_t a __attribute__((unused)) = 0;                               \
    uint32_t b __attribute__((unused)) = 0;                               \
    uint32_t c __attribute__((unused)) = 0;                               \
    uint32_t d __attribute__((unused)) = 0;                               \
    uint32_t leaf = (MAGIC_START_WININTRIN << 16U) | MAGIC;               \
    uint32_t addr_low = (uint32_t)(((uint64_t)(*addr_ptr)) & 0xffffffff); \
    uint32_t addr_hi = (uint32_t)(((uint64_t)(*addr_ptr)) >> 32U);        \
    uint32_t size_low = (uint32_t)(((uint64_t)(*size_ptr)) & 0xffffffff); \
    uint32_t size_hi = (uint32_t)(((uint64_t)(*size_ptr)) >> 32U);        \
    __cpuid(leaf, addr_low, a, b, c, d);                                  \
    __cpuid(leaf, addr_hi, a, b, c, d);                                   \
    __cpuid(leaf, size_low, a, b, c, d);                                  \
    __cpuid(leaf, size_hi, a, b, c, d);                                   \
    *(uint64_t *)addr_ptr = 0;                                            \
    *(uint64_t *)addr_ptr |= a;                                           \
    *(uint64_t *)addr_ptr |= (((uint64_t)b) << 32U);                      \
    *(uint64_t *)size_ptr = 0;                                            \
    *(uint64_t *)size_ptr |= c;                                           \
    *(uint64_t *)size_ptr |= (((uint64_t)d) << 32U);                      \
  } while (0)
...
int UefiMain(void *imageHandle, EfiSystemTable *SystemTable) {
  ...
  __arch_harness_start(&buffer_ptr, &size);

  if (size > sizeof(buffer) - 1) {
      SystemTable->conOut->output_string(SystemTable->conOut,
                                         (int16_t *)L"BAD BAD BAD BAD BAD BAD\r\n");
      __arch_harness_stop();
      return 0;
  }
