...
UefiMain(IN EFI_HANDLE ImageHandle, IN EFI_SYSTEM_TABLE *SystemTable) {
  UINTN input_size = 64;
  UINTN *input_size_ptr = &input_size;
  UINT8 *input = (UINT8 *)AllocatePages(EFI_SIZE_TO_PAGES(input_size));
  ...
  HARNESS_START(input, input_size_ptr);
  ...
  if (input) {
    FreePages(input, EFI_SIZE_TO_PAGES(input_size));
  }
