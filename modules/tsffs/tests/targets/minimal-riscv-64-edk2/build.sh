...
IMAGE_NAME="edk2-build-tsffs-gcc-riscv64-test"
CONTAINER_UID=$(echo "${RANDOM}" | sha256sum | head -c 8)
CONTAINER_NAME="${IMAGE_NAME}-tmp-${CONTAINER_UID}"

pushd "${SCRIPT_DIR}" || exit 1

cp "${SCRIPT_DIR}/../../../../../harness/tsffs-gcc-riscv64.h" "${SCRIPT_DIR}/src/tsffs-gcc-riscv64.h"

docker build -t "${IMAGE_NAME}" -f "Dockerfile" .
docker create --name "${CONTAINER_NAME}" "${IMAGE_NAME}"
docker cp \
    "${CONTAINER_NAME}:/edk2/HelloWorld/Build/HelloWorld/DEBUG_GCC/RISCV64/HelloWorld.efi" \
    "${SCRIPT_DIR}/test.efi"
