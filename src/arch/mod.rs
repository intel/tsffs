...
pub(crate) enum ArchitectureHint {
    ...
    Aarch64,
    /// The architecture is PPE42 (PowerPC Processor Embedded 42)
    Ppe42,
}
...
pub(crate) enum Architecture {
    ...
    /// The PPE42 architecture (PowerPC Processor Embedded 42)
...
impl ArchitectureOperations for Architecture {
    ...
    where
        Self: Sized,
    {
        // CRITICAL: Check PPE42 FIRST before x86, as x86 check is very permissive
        match PPE42ArchitectureOperations::new(cpu) {
            Ok(ppe42) => return Ok(Self::Ppe42(ppe42)),
            Err(_) => {} // Try other architectures
        }
