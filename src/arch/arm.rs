...
pub(crate) struct ARMArchitectureOperations {
    cpu: *mut ConfObject,
...
impl ArchitectureOperations for ARMArchitectureOperations {
    ...
    fn new(cpu: *mut ConfObject) -> Result<Self> {
        ...
        if arch == "arm" || arch == "armv7" || arch == "armv6" || arch == "armv5" || arch == "arm32"
        {
            ...
            bail!("Architecture {} is not arm", arch);
        }
    }
...
impl ARMArchitectureOperations {
...
impl TracerDisassembler for Disassembler {
    ...
    fn last_was_control_flow(&self) -> bool {
        if let Some(last) = self.last.as_ref() {
            ...
            return matches!(last.opcode, Opcode::B);
        }
    ...
    fn last_was_call(&self) -> bool {
        if let Some(last) = self.last.as_ref() {
            return matches!(last.opcode, Opcode::BL | Opcode::BLX);
        }
    ...
    fn last_was_ret(&self) -> bool {
        if let Some(last) = self.last.as_ref() {
            if (last.opcode == Opcode::POP
                && last.operands.iter().any(|o| match o {
                    Operand::Reg(r) => r.number() == 15,
                    Operand::RegList(l) => l & (1 << 15) != 0,
                    _ => false,
                }))
                || (last.opcode == Opcode::MOV
                    && last
                        .operands
                        .first()
                        .is_some_and(|f| matches!(f, Operand::Reg(r) if r.number() == 15)))
            {
                true
            } else {
                last.opcode == Opcode::BX
            }
        } else {
            false
        }
    ...
    fn last_was_cmp(&self) -> bool {
        if let Some(last) = self.last.as_ref() {
            return matches!(
                last.opcode,
                Opcode::CMN | Opcode::CMP | Opcode::TST | Opcode::TEQ
            );
        }
        ...
        if self.last_was_cmp() {
            if let Some(last) = self.last.as_ref() {
                for operand in &last.operands {
                    match operand {
                        Operand::Reg(r) => {
                            cmp_exprs.push(CmpExpr::Reg((format!("r{}", r.number()), 0)));
                        }
                        Operand::RegWBack(r, _w) => {
                            cmp_exprs.push(CmpExpr::Reg((format!("r{}", r.number()), 0)));
                        }
                        Operand::RegList(l) => {
                            for i in 0..16 {
                                if l & (1 << i) != 0 {
                                    cmp_exprs.push(CmpExpr::Reg((format!("r{}", i), 0)));
                                }
                            }
                        }
                        Operand::RegDeref(r) => {
                            cmp_exprs.push(CmpExpr::Deref((
                                Box::new(CmpExpr::Reg((format!("r{}", r.number()), 0))),
                                None,
                            )));
                        }
                        Operand::RegShift(rs) => match rs.into_shift() {
                            RegShiftStyle::RegImm(r) => {
                                let reg = r.shiftee();
                                let imm = r.imm();
                                let typ = match r.stype() {
                                ...
                                cmp_exprs.push(CmpExpr::Shift((
                                    Box::new(CmpExpr::Reg((format!("r{}", reg.number()), 0))),
                                    Box::new(CmpExpr::U8(imm)),
                                    typ,
                                )));
                            }
                            RegShiftStyle::RegReg(r) => {
                                let shiftee = r.shiftee();
                                let by = r.shifter();
                                let typ = match r.stype() {
                                    ShiftStyle::LSL => CmpExprShift::Lsl,
                                    ShiftStyle::LSR => CmpExprShift::Lsr,
                                ...
                                cmp_exprs.push(CmpExpr::Shift((
                                    Box::new(CmpExpr::Reg((format!("r{}", shiftee.number()), 0))),
                                    Box::new(CmpExpr::Reg((format!("r{}", by.number()), 0))),
                                    typ,
                                )));
                            }
                        },
                        Operand::RegDerefPostindexRegShift(r, _, _, _) => {
                            cmp_exprs.push(CmpExpr::Deref((
                                Box::new(CmpExpr::Reg((format!("r{}", r.number()), 0))),
                                None,
                            )));
                        }
                        Operand::RegDerefPreindexRegShift(r, rs, a, _) => match rs.into_shift() {
                            RegShiftStyle::RegImm(ri) => {
                                let reg = ri.shiftee();
                                let imm = ri.imm();
                                let typ = match ri.stype() {
                                    ShiftStyle::LSL => CmpExprShift::Lsl,
                                    ShiftStyle::LSR => CmpExprShift::Lsr,
                                    ShiftStyle::ASR => CmpExprShift::Asr,
                                    ShiftStyle::ROR => CmpExprShift::Ror,
                            ...
                            RegShiftStyle::RegReg(ri) => {
                                let shiftee = ri.shiftee();
                                let by = ri.shifter();
                                let typ = match ri.stype() {
                                    ShiftStyle::LSL => CmpExprShift::Lsl,
                                    ShiftStyle::LSR => CmpExprShift::Lsr,
                        ...
                        Operand::RegDerefPostindexOffset(r, _, _, _) => {
                            cmp_exprs.push(CmpExpr::Deref((
                                Box::new(CmpExpr::Reg((format!("r{}", r.number()), 0))),
                                None,
                            )));
                        }
                        Operand::RegDerefPreindexOffset(r, i, a, _) => {
                            cmp_exprs.push(CmpExpr::Deref((
                                if *a {
                                    Box::new(CmpExpr::Add((
                                        Box::new(CmpExpr::Reg((format!("r{}", r.number()), 0))),
                                        Box::new(CmpExpr::U16(*i)),
                                    )))
                                } else {
                                    Box::new(CmpExpr::Sub((
                                        Box::new(CmpExpr::Reg((format!("r{}", r.number()), 0))),
                        ...
                        Operand::RegDerefPostindexReg(r, _, _, _) => {
                            cmp_exprs.push(CmpExpr::Deref((
                                Box::new(CmpExpr::Reg((format!("r{}", r.number()), 0))),
                                None,
                            )));
                        }
                        Operand::RegDerefPreindexReg(r, i, a, _) => {
                            cmp_exprs.push(CmpExpr::Deref((
                                if *a {
                                    Box::new(CmpExpr::Add((
                                        Box::new(CmpExpr::Reg((format!("r{}", r.number()), 0))),
                                        Box::new(CmpExpr::Reg((format!("r{}", i.number()), 0))),
                                    )))
                                } else {
                                    Box::new(CmpExpr::Sub((
                                        Box::new(CmpExpr::Reg((format!("r{}", r.number()), 0))),
                        ...
                        Operand::Imm12(i) => {
                            cmp_exprs.push(CmpExpr::U16(*i));
                        }
                        Operand::Imm32(i) => {
                            cmp_exprs.push(CmpExpr::U32(*i));
                        }
                        _ => {}
                    }
                }
        ...
        if self.last_was_cmp() {
            if let Some(last) = self.last.as_ref() {
                return match last.opcode {
                    Opcode::CMP => vec![CmpType::Equal, CmpType::Lesser, CmpType::Greater],
                    Opcode::CMN => vec![CmpType::Equal, CmpType::Lesser, CmpType::Greater],
                    Opcode::TST => vec![CmpType::Equal, CmpType::Lesser, CmpType::Greater],
                    Opcode::TEQ => vec![CmpType::Equal],
                    _ => vec![],
                };
            }
        }
