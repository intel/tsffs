...
pub(crate) struct PPE42ArchitectureOperations {
    ...
    cpu_instruction_query: Option<CpuInstructionQueryInterface>,
    #[allow(dead_code)]
    cpu_instrumentation_subscribe: Option<CpuInstrumentationSubscribeInterface>,
...
impl ArchitectureOperations for PPE42ArchitectureOperations {
    ...
    fn new(cpu: *mut ConfObject) -> Result<Self> {
        ...
        if arch == "ppc" || arch == "powerpc" || arch == "ppe42" || arch == "ppc32" {
            ...
            // CpuInstructionQueryInterface is optional - not all PPE42 CPUs support it
            let cpu_instruction_query: Option<CpuInstructionQueryInterface> = get_interface(cpu).ok();
            // CpuInstrumentationSubscribeInterface is optional - not all PPE42 CPUs support it
            let cpu_instrumentation_subscribe: Option<CpuInstrumentationSubscribeInterface> = get_interface(cpu).ok();
        ...
        Self: Sized,
    {
        Ok(Self {
            ...
            cpu_instruction_query: get_interface(cpu).ok(),
            cpu_instrumentation_subscribe: get_interface(cpu).ok(),
    ...
    fn trace_pc(&mut self, instruction_query: *mut instruction_handle_t) -> Result<TraceEntry> {
        // If CpuInstructionQueryInterface is not available, we can't trace PC
        let cpu_instruction_query = self.cpu_instruction_query.as_mut()
            .ok_or_else(|| anyhow!("CpuInstructionQueryInterface not available for trace_pc"))?;
        
        let instruction_bytes = cpu_instruction_query.get_instruction_bytes(instruction_query)?;
    ...
    fn trace_cmp(&mut self, instruction_query: *mut instruction_handle_t) -> Result<TraceEntry> {
        // If CpuInstructionQueryInterface is not available, we can't trace CMP
        let cpu_instruction_query = self.cpu_instruction_query.as_mut()
            .ok_or_else(|| anyhow!("CpuInstructionQueryInterface not available for trace_cmp"))?;
