...
use crate::{
    ...
    state::Stop,
    state::{ManualStart, ManualStartSize, StopReason},
    Solution, SolutionKind, Tsffs,
};
use anyhow::anyhow;
use libafl::inputs::HasBytesVec;
use simics::{
    continue_simulation, debug, error, get_processor_number, interface, lookup_file, run_alone,
    version, AsConfObject, AttrValue, BreakpointId, ConfObject, GenericAddress, Result,
};
use std::{
    ffi::{c_char, CStr},
    fs::read,
...
impl Tsffs {
    ...
    pub fn set_start_on_harness(&mut self, start_on_harness: bool) -> Result<()> {
        ...
        self.start_on_harness = start_on_harness;
    ...
    pub fn set_stop_on_harness(&mut self, stop_on_harness: bool) -> Result<()> {
        ...
        self.stop_on_harness = stop_on_harness;
    ...
    #[interface(name = "start_with_maximum_size")]
    ...
    pub fn start_with_maximum_size_legacy(
        &mut self,
        cpu: *mut ConfObject,
        testcase_address: GenericAddress,
        maximum_size: u32,
    ...
    #[interface(name = "start_without_buffer")]
    /// Interface method to manually start the fuzzing loop by taking a snapshot, saving
    /// the testcase and maximum testcase size and resuming execution of the simulation.
    /// This method does not need to be called if `set_start_on_harness` is enabled.
    ///
    ...
    pub fn start_without_buffer_legacy(&mut self, cpu: *mut ConfObject) -> Result<AttrValue> {
        ...
        testcase.testcase.bytes().to_vec().try_into()
    }
    ...
    pub fn stop_legacy(&mut self) -> Result<()> {
    ...
    pub fn solution_legacy(&mut self, id: u64, message: *mut c_char) -> Result<()> {
    ...
    /// has been found.
    pub fn repro_legacy(&mut self, testcase_file: *mut c_char) -> Result<()> {
    ...
    pub fn set_all_exceptions_are_solutions(&mut self, all_exceptions_are_solutions: bool) {
        ...
        self.all_exceptions_are_solutions = all_exceptions_are_solutions;
    }
    ...
    pub fn set_all_breakpoints_are_solutions(&mut self, all_breakpoints_are_solutions: bool) {
        ...
        self.all_breakpoints_are_solutions = all_breakpoints_are_solutions;
    }
    ...
    pub fn set_generate_random_corpus(&mut self, generate_random_corpus: bool) -> Result<()> {
        ...
        self.generate_random_corpus = generate_random_corpus;
    ...
    pub fn tokenize_executable(&mut self, executable_file: *mut c_char) -> Result<()> {
        ...
        self.token_executables.push(executable_path);
    ...
    pub fn tokenize_src(&mut self, source_file: *mut c_char) -> Result<()> {
        ...
        self.token_src_files.push(source_path);
    ...
    pub fn add_token_file(&mut self, token_file: *mut c_char) -> Result<()> {
        ...
        if token_file.is_file() {
            self.token_files.push(token_file);
        }

        Ok(())
    }

    #[interface(name = "add_trace_processor")]
    /// Add a processor to be traced. By default, only the processor the start event occurs on
    /// is used for tracing.
    pub fn add_trace_processor_legacy(&mut self, cpu: *mut ConfObject) -> Result<()> {
        debug!(
            self.as_conf_object(),
    ...
    #[interface(name = "add_architecture_hint")]
    ...
    pub fn add_architecture_hint_legacy(
        &mut self,
        cpu: *mut ConfObject,
        hint: *mut c_char,
    ) -> Result<()> {
