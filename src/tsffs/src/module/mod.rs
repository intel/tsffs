...
use simics::api::{
    ...
    continue_simulation_alone, discard_future, free_attribute, get_object, get_processor_number,
    hap_add_callback, init_command_line, last_error, log_info, main_loop, quit, register_interface,
    restore_micro_checkpoint, run_alone, run_command, save_micro_checkpoint, AttrValue, ConfObject,
    Hap, HapCallback, MicroCheckpointFlags, SimException, SimicsClassCreate, SimicsModule,
};
...
impl Module {
    ...
    #[params(!slf: *mut simics::api::ConfObject, ...)]
...
pub extern "C" fn init_local() {
    log_info(
        1,
        get_object("sim").unwrap_or_else(|e| panic!("Unable to get object named 'conf': {e}")),
        "TSFFS Module Loaded",
    )
    .unwrap_or_else(|e| panic!("Unable to log load message: {e}"));
