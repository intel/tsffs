...
int Check2(char *buffer, EfiSystemTable *SystemTable) {
  for (int i = 0; i < 8; i++) {
    if (buffer[i] != password2[i]) {
      return 1;
    }
  }
  SystemTable->conOut->output_string(
      SystemTable->conOut, (int16_t *)L"All characters were correct!!\r\n");
  uint8_t *ptr = (uint8_t *)0xffffffffffffffff;
  *ptr = 0;
