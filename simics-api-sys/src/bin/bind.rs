...
use std::{
    ...
    ffi::OsStr,
    fs::{create_dir_all, read, read_dir, OpenOptions},
...
// The ISPM downloaded tarballs. Replace and update this file when new versions of the package
// manager are released. Right now, this requires the *internal* ISPM, but we need to change that
// requirement if we ever want to open source this project
// TODO: Use the public ISPM instead! Right now, it gives a keyfile error because we try to
// install private packages
const _SIMICS_PACKAGE_MANAGER_TARBALL_PUBLIC: &[u8] =
    include_bytes!("./resource/intel-simics-package-manager-cli-1.7.0-linux64.tar.gz");
const SIMICS_PACKAGE_MANAGER_TARBALL_PRIVATE: &[u8] =
    include_bytes!("./resource/ispm-internal-cli-1.7.0-linux64.tar.gz");
...
fn generate_bindings(args: &Args) -> Result<()> {
    ...
    for (base_version, (wrapper, base_path)) in include_wrappers {
        ...
        let header_path = args.bindings_dir.join(format!("simics-{}.h", base_version));
...
fn install_packages(args: &Args) -> Result<()> {
    ...
    let tar = GzDecoder::new(BufReader::new(SIMICS_PACKAGE_MANAGER_TARBALL_PRIVATE));
...
fn generate_mod(args: &Args) -> Result<()> {
    let versions = args
        ...
        .filter_map(|fname| {
            ...
            fname_parts.get(1).map(|part| part.to_string())
        })
        .collect::<Vec<_>>();
    ...
    for version in versions {
        mod_text += &format!(r#"#[cfg(feature = "{}")]\n"#, version);
        mod_text += &format!(r#"include!("bindings-{}.rs")\n"#, version);
    }
