...
FORCE_INLINE branch_tracer_t *
branch_tracer_of_obj(conf_object_t *obj) { return (branch_tracer_t *)obj; }
        ...
        void *user_data)
{
        ...
        tuple_int_string_t da = tracer->pi_iface->disassemble(
                cpu, 0, data, 0);
        SIM_attr_free(&data);
        ...
        for (int i = 0; i < ALEN(branch_infos); i++) {
                ...
                if (strncmp(da.string, sb_str(&sb), sb_len(&sb)) == 0) {
                        //if (strcmp(branch_infos[i].type, "ret")==0) SIM_log_info(1, tracer, 0, "Installing on %llx", tracer->iq_iface->logical_address(cpu, iq_handle));
...
static set_error_t
set_processor_attribute(conf_object_t *obj, attr_value_t *val)
{
        branch_tracer_t *tracer = branch_tracer_of_obj(obj);
        conf_object_t* processor = SIM_attr_object_or_nil(*val);
        
        if (processor){
          //TODO: do tracer setup. if it fails we do not have a proc and do not set the attr
          tracer->cpu_iface      = SIM_C_GET_INTERFACE(processor, cpu_instrumentation_subscribe);
          ...
          if ( (tracer->cpu_iface == NULL) || (tracer->iq_iface == NULL) || (tracer->ci_iface == NULL) || (tracer->pi_iface == NULL) ){
              SIM_LOG_ERROR(obj, 0, "Provided attribute is not providing required interfaces.");
              return Sim_Set_Interface_Not_Found;
          }
          tracer->cpu_iface->register_cached_instruction_cb(processor, NULL, cached_instruction_cb, tracer);
        }
...
static attr_value_t
get_processor_attribute(conf_object_t *obj)
{
        branch_tracer_t *tracer = branch_tracer_of_obj(obj);
...
static set_error_t
set_shmem(void *param, conf_object_t *obj, attr_value_t *val,
               attr_value_t *idx)
{
        branch_tracer_t *tracer = branch_tracer_of_obj(obj);
...
init_branch_tracer_class(void)
{
        ...
        conf_class_t *cl = SIM_register_class("afl_branch_tracer",
                                              &funcs);
        SIM_register_attribute(
                cl, "processor",
                get_processor_attribute, set_processor_attribute,
                Sim_Attr_Pseudo, "o|n",
                "The <i>processor</i> to trace.");
        SIM_register_typed_attribute(cl, "shm_name",
                                     NULL, NULL,
                                     set_shmem, NULL,
