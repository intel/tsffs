...
const EDK2_REPO_HASH: &str = "d189de3b0a2f44f4c9b87ed120be16569ea19b51";
...
fn build_efi_module() -> Result<()> {
    ...
    .run(RUN![
        "stuart_setup",
        "-c",
        "/edk2/X509Parse/PlatformBuild.py",
        "TOOL_CHAIN_TAG=GCC5"
    ])
    .run(RUN![
        "stuart_update",
    ...
    .run(RUN![
        ...
        "source /edk2/edksetup.sh \
            && stuart_build -c /edk2/X509Parse/PlatformBuild.py \
               TOOL_CHAIN_TAG=GCC5 EDK_TOOLS_PATH=/edk2/BaseTools/ \
            || ( cat /edk2/X509Parse/Build/BUILDLOG.txt && exit 1 )"
    ]);
    ...
    let hello_world_efi_out_path = out_dir()?.join("X509Parse.efi");
    ...
    let container_name_suffix: String = rand::thread_rng()
        ...
        .map(char::from)
        .collect();
    let container_name = format!("edk2-build-x509-parse-tmp-{}", container_name_suffix);
    ...
    let cp_status = Command::new("docker")
        .arg("cp")
        .arg(format!(
            "{}:/edk2/X509Parse/Build/CryptoPkg/All/DEBUG_GCC5/X64/X509Parse.efi",
            &container_name
        ))
