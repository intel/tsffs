...
static confuse_ll *confuse_ll_of_obj(conf_object_t *obj) {
...
void restore_and_clear(lang_void *data) {
    ...
    if (kill(run_ctrl->if_pid, SIGUSR2)) {
        SIM_log_error((conf_object_t *)data, 0,
                      "Could not send SIGUSR2 to pid %d from restore and clear handler",
                      run_ctrl->if_pid);
    }
}
...
static void usr2_sig_handler(int signum) {
    ...
    if (obj == NULL)
        obj = SIM_get_object("fuzz_if"); // FIXME: Hard coded object name is a "no no"!
    SIM_run_alone(restore_and_clear, obj);
}

static set_error_t trigger_usr2(void *param, conf_object_t *obj, attr_value_t *val,
                                attr_value_t *idx) {
...
static void stop_callback(lang_void *callback_data, conf_object_t *trigger_obj,
                          int64 exception, char *error_string) {
    ...
    if (run_ctrl->cnt > 0)
      ...
      */
    if (kill(run_ctrl->if_pid, SIGUSR2)) {
        SIM_log_error((conf_object_t *)callback_data, 0,
                      "Could not send SIGUSR2 to pid %d from stop handler",
                      run_ctrl->if_pid);
    }
}

static set_error_t arm_usr2_on_stop(void *param, conf_object_t *obj, attr_value_t *val,
                                    attr_value_t *idx) {
    confuse_ll *run_ctrl = confuse_ll_of_obj(obj);
    run_ctrl->if_pid = (pid_t)SIM_attr_integer(*val);
    if (run_ctrl->if_pid) {
...
static conf_object_t *run_ctrl_alloc_object(void *data) {
    confuse_ll *run_ctrl = MM_ZALLOC(1, confuse_ll);
    return &run_ctrl->obj;
}

static void *run_ctrl_init_object(conf_object_t *obj, void *param) {
    confuse_ll *run_ctrl = confuse_ll_of_obj(obj);
...
void init_local(void) {
    class_data_t cdata = {
        ...
        .class_desc = "Insert SIGUSR1 and SIGUSR2 handlers for Simics",
        .description = "A class that install SIGUSR1 and SIGUSR2 handlers"
                       "into Simics. Only one such object is allowed to exist.",
    };
    conf_class_t *cl = SIM_register_class(CLASS_NAME, &cdata);
    SIM_register_typed_attribute(cl, "send_usr2", NULL, NULL, trigger_usr2, NULL,
                                 Sim_Attr_Pseudo, "i", NULL,
                                 "Send SIGUSR2 to a process");
    SIM_register_typed_attribute(cl, "arm_auto_send_usr2", NULL, NULL, arm_usr2_on_stop,
                                 NULL, Sim_Attr_Pseudo, "i", NULL,
