...
static confuse_dio *confuse_dio_of_obj(conf_object_t *obj) {
...
/* Writer protocol callback function.
   Called when the target reads from the pipe and we are supposed to write something
   into it.
 */
static void pipe_agent_writer(conf_object_t *cpu, uintptr_t bufh, uint64 magic) {
    ...
    if (man->shm) {
        ...
        if (len > 0) {
            ...
            if (buf.len < len) {
                SIM_log_error((conf_object_t *)man, 0,
                              "Magic pipe buffer too small (%ld)!", buf.len);
                return;
            }
        ...
        SIM_log_info(1, (conf_object_t *)man, 0,
                     "SHM not initialized. Not pushing any data into magic pipe");
    }
}

/* Reader protocol callback function.
   Called when the target has written into the pipe and we are supposed to take data
   out.
*/
static void pipe_agent_reader(conf_object_t *cpu, uintptr_t bufh, uint64 magic) {
    confuse_dio *man = confuse_dio_of_obj(NULL);
    man->haps++;
    ...
    if (len > 0) { // must be end of test
        ...
        if (man->shm) {
            ...
            SIM_log_info(
                1, (conf_object_t *)man, 0,
                "SHM not initialized. Not pushing any data from magic pipe into SHM.");
        }
    }
...
/* Connect to the magic pipe, by registering callbacks for some magic
   numbers. */
static set_error_t connect_to_pipe(confuse_dio *man, conf_object_t *pipe) {
    ...
    /* register a reader and writer function for the reserved magic
       number. */
    psu->register_reserved_pipe(pipe, &man->obj, man->magic, pipe_agent_reader,
                                pipe_agent_writer);
    return Sim_Set_Ok;
}

/* Disconnect from the magic pipe and close all input and output files. */
static void disconnect_pipe(confuse_dio *man) {
...
static conf_object_t *dio_alloc_object(void *data) {
    confuse_dio *man = MM_ZALLOC(1, confuse_dio);
    return &man->obj;
}

/* Initialize the pipe manager object instance. */
static void *dio_init_object(conf_object_t *obj, void *param) {
...
static attr_value_t dio_get_pipe(void *param, conf_object_t *obj, attr_value_t *idx) {
    confuse_dio *man = confuse_dio_of_obj(obj);
    return SIM_make_attr_object(man->pipe);
}

/* Set the pipe to be used. */
static set_error_t dio_set_pipe(void *param, conf_object_t *obj, attr_value_t *val,
                                attr_value_t *idx) {
    confuse_dio *man = confuse_dio_of_obj(obj);
...
static attr_value_t dio_get_haps(void *param, conf_object_t *obj, attr_value_t *idx) {
    confuse_dio *man = confuse_dio_of_obj(obj);
    return SIM_make_attr_uint64(man->haps);
}

/* Set the hap count attribute. Only useful for debugging purposes. */
static set_error_t dio_set_haps(void *param, conf_object_t *obj, attr_value_t *val,
                                attr_value_t *idx) {
    confuse_dio *man = confuse_dio_of_obj(obj);
...
/* Get the hap count attribute */
static attr_value_t dio_get_magic(void *param, conf_object_t *obj, attr_value_t *idx) {
    confuse_dio *man = confuse_dio_of_obj(obj);
    return SIM_make_attr_uint64(man->magic);
}

/* Set the hap count attribute. Only useful for debugging purposes. */
static set_error_t dio_set_magic(void *param, conf_object_t *obj, attr_value_t *val,
                                 attr_value_t *idx) {
    confuse_dio *man = confuse_dio_of_obj(obj);
    if (man->pipe)
...
static set_error_t dio_set_ifpid(void *param, conf_object_t *obj, attr_value_t *val,
                                 attr_value_t *idx) {
...
void bp_handler(lang_void *callback_data, conf_object_t *trigger_obj,
                int64 breakpoint_number, generic_transaction_t *memop) {
...
static void timeout_event_cb(conf_object_t *obj, lang_void *data) {
    confuse_dio *dio = confuse_dio_of_obj(obj);
...
void add_abnormal_exit(conf_object_t *obj, breakpoint_id_t bp, const char *message) {
    ...
    tmp->hap =
        SIM_hap_add_callback_index("Core_Breakpoint_Memop", bp_handler, tmp, tmp->bp);
}
...
void init_local(void) {
    class_data_t cdata = {
        ...
        .class_desc = "forwards data through a magic pipe connection",
        .description = "A class that receives and transmits data through a magic pipe"
                       " connection to a pipe agent running in the target system.",
    };
    conf_class_t *cl = SIM_register_class(DIO_CLASS_NAME, &cdata);

    SIM_register_typed_attribute(cl, "haps", dio_get_haps, NULL, dio_set_haps, NULL,
                                 Sim_Attr_Optional, "i", NULL, "Magic hap count");
    SIM_register_typed_attribute(cl, "magic", dio_get_magic, NULL, dio_set_magic, NULL,
                                 Sim_Attr_Session, "i", NULL,
                                 "Magic number of the pipe agent");
    SIM_register_typed_attribute(cl, "if_pid", NULL, NULL, dio_set_ifpid, NULL,
                                 Sim_Attr_Pseudo, "i", NULL,
                                 "Inform device about interface PID");
    SIM_register_typed_attribute(cl, "pipe", dio_get_pipe, NULL, dio_set_pipe, NULL,
    ...
    timeout_event = SIM_register_event("timeout_event", cl, 0, timeout_event_cb, NULL,
                                       NULL, NULL, NULL);
}
