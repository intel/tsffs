 ...
 * indicates that the register `ARG0` and `ARG1` (as defined per platform) are
 * set to the buffer pointer and a pointer to the buffer size, respectively.
 */
 ...
 * Instruction](https://simics-download.pdx.intel.com/simics-6/docs/html/simics-user-guide/breakpoints.html#Magic-Breakpoints)
 * indicates that execution should stop, and the root fuzzing snapshot should be
 * restored
 */
#define MAGIC_STOP 2

/**
 * MAGIC_START_WININTRIN, when passed as the value of `n` in a magic
 * instruction, indicates a magic start sequence that is supported by X64
 * windows intrinsic `__cpuidex`. This signals the start of the following
 * sequence of cpuid (as this is only supported on X64) instructions:
 * - cpuid eax=[(MAGIC_START_WININTRIN << 16U) | MAGIC] ecx=(BUFFER_PTR &
 * 0xffffffff)
 * - cpuid eax=[(MAGIC_START_WININTRIN << 16U) | MAGIC] ecx=((BUFFER_PTR >> 32U)
 * & 0xffffffff)
 * - cpuid eax=[(MAGIC_START_WININTRIN << 16U) | MAGIC] ecx=(SIZE & 0xffffffff)
 * - cpuid eax=[(MAGIC_START_WININTRIN << 16U) | MAGIC] ecx=((SIZE >> 32U) &
 * 0xffffffff)
 *
 * The first three `__cpuid` calls output no value into the `cpuInfo` buffer.
 * The fourth sets `cpuInfo` to:
 *
 * ```c
 *  {
...
#define __arch_harness_stop_extended(val_ptr) __arch_harness_stop()
